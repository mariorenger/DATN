...
class RESANModel(BaseModel):
    ...
    def __init__(self, hparams, iterator_creator, seed=None):
        """Initialization steps for ReSAN.
        Compared with the BaseModel, ReSAN need word embedding.
        After creating word embedding matrix, BaseModel's __init__ method will be called.

        Args:
            hparams (object): Global hyper-parameters. Some key setttings such as filter_num are there.
            iterator_creator_train (object): ReSAN data loader class for train data.
            iterator_creator_test (object): ReSAN data loader class for test and validation data
        """
    ...
    def _build_graph(self):
        ...
        model, scorer = self._build_resan()
        return model, scorer

    def _build_userencoder(self, newsencoder):
        """The main function to create user encoder of ReSAN.

        Args:
            newsencoder (object): the news encoder of ReSAN.

        Return:
            object: the user encoder of ReSAN.
        """
        ...
        y = SelfAttention(hparams.head_num, hparams.head_dim, name="self_attention", seed=self.seed)(
    ...
    def _build_mask(self, newsencoder):
        """The main function to create user encoder of ReSAN.
        ...
        model = keras.Model(his_input_title_body_verts, y, name="user_encoder1")

        self.mask = model.get_layer("self_attention").mask
        self.rw_loss = model.get_layer("self_attention").rl_loss

        return model

    def _build_newsencoder(self, embedding_layer):
        """The main function to create news encoder of ReSAN.
        ...
        Return:
            object: the news encoder of ReSAN.
        """
        hparams = self.hparams
        ...
        news_repr = AttLayer2(hparams.attention_hidden_dim, seed=self.seed, name="weight_attention")(
            concate_repr
        )

        model = keras.Model(input_title_body_verts, news_repr, name="news_encoder")
        self.news_att_w = model.get_layer("weight_attention").att_w
    ...
    def _build_vertencoder(self):
        """build vert encoder of ReSAN news encoder.

        Return:
            object: the vert encoder of ReSAN.
        """
        hparams = self.hparams
    ...
    def _build_subvertencoder(self):
        """build subvert encoder of ReSAN news encoder.

        Return:
            object: the subvert encoder of ReSAN.
        """
        hparams = self.hparams
    ...
    def _build_resan(self):
        """The main function to create ReSAN's logic. The core of ReSAN
        ...
        #hannn
        self.getmask = self._build_mask(self.newsencoder)
