...
class BaseModel:
    ...
    def _get_opt(self):
        ...
        if optimizer == "adam":
            train_opt = tf.keras.optimizers.legacy.Adam(lr=lr)
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            for batch_data_input in tqdm_util:
                ...
                if hasattr(self.att, 'ln'):
                    ...
                    layer[0].assign_sub(tf.fill(layer[0].shape, 0.001*(step_data_loss+self.att.rl_loss)))
                    layer[1].assign_sub(tf.fill(layer[1].shape, 0.001*(step_data_loss+self.att.rl_loss)))
    ...
    def news(self, batch_news_input):
        ...
        # feed_dict = {self.newsencoder.input: news_input}
        # att_w = self.sess.run([self.news_att_w], feed_dict=feed_dict)
